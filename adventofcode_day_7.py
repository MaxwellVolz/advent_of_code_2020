...
bag_rules = """dark orange bags contain 3 dark chartreuse bags.
...
def sum_inner_bags(bag, bag_multiplier=0):
  for rule in bag_rules:
    if f'{bag} bags contain' in rule and 'other bags.' not in bag:
      print(rule)
      ...
      while bag_iter*4 < len(inner_bags):
        ...
        inner_bag = f'{inner_bags[increment+1]} {inner_bags[increment+2]}'
        print(inner_bag, inner_bags[increment],bag_multiplier)
        if "other bags." not in inner_bag:
          bag_multiplier = int(bag_multiplier)+int(inner_bags[increment])
          print("sum_out:",sum_inner_bags(inner_bag, bag_multiplier))
        else:
          pass
  ...
  print('bag_multiplier', bag_multiplier)

  return f"{bag} {bag_multiplier}"
...
for rule in bag_rules:
  if 'shiny gold bags contain' in rule:
    ...
    while bag_iter*4 < len(inner_bags):
      increment = bag_iter * 4
      inner_bag = f'{inner_bags[increment+1]} {inner_bags[increment+2]}'
      # print(inner_bag, inner_bags[increment])

      bag_count += int(sum_inner_bags(inner_bag, inner_bags[increment]).split()[2])
